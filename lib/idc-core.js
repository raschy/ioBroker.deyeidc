...
module.exports = class idcCore {
	...
	checkDataFrame(data) {
		...
		if (debugging) console.log(`(f) checkDataFrame SequenzNummer: ${seqNr}`);
			...
			for (let i = 0; i < modbus_data.length / 2; i++) {
				...
				if (resultFilter.length > 0) {
					for (let j = 0; j < resultFilter.length; j++) {
						...
						switch (result.rules) {
							case 1: {	// für 16-bit-unsigned Werte
							...
							case 2: {	// für 16-bit-signed Werte
							...
							case 3: {	// für 32-bit-unsigned Werte
							...
							case 4: {	// für 32-bit-signed Werte
