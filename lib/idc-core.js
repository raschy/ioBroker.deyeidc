...
module.exports = class idcCore {
	...
	checkDataFrame(data) {
		...
		if (!data) {	//obsoled, if firmware 2.06 is ok
			throw new MyError({ status: 'EFRDATA', message: `No data received ${bufferInHex}` });
		}		//
	...
	*/
		if (start != '165') { // Only evaluations when start bit = a5
			return {
				register: -1,
		...
		if (msgLen > 1) {	//> 1
			...
			console.log('>>>>', bufferInHex.readUInt32LE(13));
...
*/
	...
	modbusFrame(i) {
		...
		buffer.writeUInt16BE(0x0103, 0); //0x0103
		buffer.writeUInt16BE(firstregister, 2);
		buffer.writeUInt16BE(lastregister - firstregister + 2, 4);	//One more register due to double buffer access
