...
module.exports = class idcCore {
	...
	checkDataFrame(data) {
		...
		if (debugging) console.log(`(f) checkDataFrame StartByte: ${startByte}`);
		...
		if (debugging) console.log(`(f) checkDataFrame length of data: ${dataLen}`);
		...
		if (logging) this.log.silly(`[#checkDataFrame] Messagelength: ${modbusLen}`);
	...
	readCoils(data) {
		...
		//
		if (data.register > 0) {
			if (logging) this.log.silly(`[#readCoils] Data.Register: ${data.register} ${Object.keys(this.Registers).length}`);
			...
			if (debugging) console.log('ModbusLength: ', modbus_data.length);
			if (logging) this.log.silly(`[#readCoils] ModbusLength: ${modbus_data.length}`);

			if (logging) this.log.silly(`[*readCoils] Data.Register: ${JSON.stringify(this.Registers[data.register - 1])}`);
			if (logging) this.log.silly(`[*readCoils] RegisterStart: ${this.Registers[data.register - 1].registerStart}`);

			for (let i = 0; i < modbus_data.length / 2; i++) {
				//if (logging) this.log.silly(`[##readCoils] I ===: ${i}`);
				const resultFilter = this.Coils.filter((obj) => {
					//if (logging) this.log.silly(`[##readCoils] Object: ${JSON.stringify(obj)}`);
					//if (logging) this.log.silly(`[##readCoils] ResultFilter: ${this.Registers[data.register - 1].registerStart} | I=${i}`);
					return obj.register === this.Registers[data.register - 1].registerStart + i;
				});
				//if (logging) this.log.silly(`#readCoils] (${i}) ResultFilter: ${JSON.stringify(resultFilter)} Register: ${this.Registers[data.register - 1].registerStart + i}`);
				//if (logging) this.log.silly(`[#readCoils] ResultFilter: ${JSON.stringify(resultFilter)}`);
				if (resultFilter.length > 0) {
					//if (logging) this.log.silly(`[###readCoils] resultFilter.length ${resultFilter.length} [${i}]`);
