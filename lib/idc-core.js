...
module.exports = class idcCore {
	...
	readCoils(data) {
		...
		const jsonResult = [];
		//
		if (logging) this.log.warn(`[*readCoils] ${JSON.stringify(this.Registers)}`);
		if (logging) this.log.warn(`[*readCoils] Len ${Object.keys(this.Registers).length} > ${data.register}`);

		if (data.register > 0 && data.register <= Object.keys(this.Registers).length) {
			if (logging) this.log.warn('[*readCoils] ********');
			...
			if (logging) this.log.silly(`[#readCoils] Startcoil: ${startCoil} ExpectedLength ${expectedLen}`);
			...
			if (modbus_data.length == expectedLen) {
				if (logging) this.log.silly('[#readCoils] works');
