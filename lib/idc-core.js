...
module.exports = class idcCore {
	...
	checkDataFrame(data) {
		...
		const startByte = parseInt(bufferInHex.readUInt8(0).toString(10)); 	// expected: 165
		if (debugging) console.log(`(f) checkDataFrame StartByte: ${startByte}`);
		if (startByte == 170 || startByte == 65) { // 0xaa => Communication with relay
			return {
				register: -1,
	...
	*/
	...
	readCoils(data) {
		...
		const jsonResult = [];
		//
		// console.log(`Registerset ##:`, data.register); //##
		if (data.register > 0) {
