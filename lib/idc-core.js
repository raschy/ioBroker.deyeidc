...
module.exports = class idcCore {
	...
	checkDataFrame(data) {
		...
		if (logging) this.log.silly(`[#checkDataFrame] Data Buffer ${this.toHexString(data)}`);
		...
		//
		const msgLen = parseInt(bufferInHex.readUInt16LE(1).toString(10));
						...
						for (let j = 0; j < resultFilter.length; j++) {
							...
							switch (result.rules) {
								case 0: {	// raw_signed
									...
									if (debugging) console.log(`Register ${startCoil + i} [0] [posBuffer:${i * 2}] ${result.name}:> ${result.key} => ${bitValue} ${result.unit} # ${result.factor}`);
								...
								case 1: {	// for 16-bit-unsigned Values
									...
									if (debugging) console.log(`Register ${startCoil + i} [1] [posBuffer:${i * 2}] ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
								...
								case 2: {	// for 16-bit-signed Values
									...
									if (debugging) console.log(`Register ${startCoil + i} [2] [posBuffer:${i * 2}] Rawdata: ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 3: {	// for 32-bit-unsigned Values
									...
									if (debugging) console.log(`Register ${startCoil + i} [3] [posBuffer:${i * 2}] ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 4: {	// for 32-bit-signed Values
									...
									const value = ((low_word + high_word * 65536) * 10 ** -result.factor).toFixed(result.factor);
									if (debugging) console.log(`Register ${startCoil + i} [4] [posBuffer:${i * 2}] ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
