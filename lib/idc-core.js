...
module.exports = class idcCore {
	...
	readCoils(data) {
		...
		if (data.register > 0) {
			...
			if (logging) this.log.silly(`[*readCoils] .Registers: ${JSON.stringify(this.Registers)}`);
			...
			for (let i = 0; i < modbus_data.length / 2; i++) {
				...
				//console.log('resultFilter: ', i, resultFilter, this.Registers[data.register - 1].registerStart + i);
				...
				if (resultFilter.length > 0) {
					...
					for (let j = 0; j < resultFilter.length; j++) {
						...
						switch (result.rules) {
							...
							case 13: {	// for 32-bit-unsigned Values
