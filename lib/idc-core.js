...
module.exports = class idcCore {
	...
	setRegisters(registers) {
		...
		//console.log(`[setRegisters]  ${JSON.stringify(this.Registers)}`);
	}

	setCoils(coils) {
		this.Coils = coils;
		//console.log(`[setCoils]  ${JSON.stringify(this.Coils)}`);
	}
	...
	checkDataFrame(data) {
		...
		if (cntrlCode != 4117) {
			...
			const fileName = './controlCode.csv';
			fs.writeFile(fileName, Date.now() + '\t' + cntrlCode + '\n', { flag: 'a' }, (err) => {
				if (err) {
	...
	*/
		if (msgLen > 1) {
			...
			if (debugging) console.log(`'(f) checkDataFrame Capture data: ${captureData} > ${this.toDate(captureData)}`);
			// Modbus
			...
			if (parseInt(offsetTime) < 1) {
				...
				const fileName = './offsetTime.csv';
				fs.writeFile(fileName, Date.now() + '\t' + offsetTime + '\n', { flag: 'a' }, (err) => {
					if (err) {
						console.log('Error: ', err);
					}
				});
	...
	readCoils(data) {
		...
		if (data.register > 0) {
			if (debugging) console.log(`Startcoil: ${this.Registers[data.register - 1].registerStart}`);
					...
					for (let j = 0; j < resultFilter.length; j++) {
						...
						switch (result.rules) {
							...
							case 4: {	// für 32-bit-signed Values
								const low_word = modbus_data.readInt16BE(i * 2);
	...
	requestFrame(req, modbus) {
		...
		//const modbus = this.modbusFrame(req);
	...
	modbusFrame(i) {
		...
		//console.log(`modbusFrame ${i} 1. <${firstregister}>  2. <${lastregister}>`);
		...
		//console.log('modbusFrame: ', buffer);1
		return buffer;
	}
