...
module.exports = class idcCore {
	...
	checkDataFrame(data) {
		...
		this.log.silly(`[#checkDataFrame] DataLength: ${dataLen}`);
		...
		this.log.silly(`[#checkDataFrame] CheckSumme_Frame: ${checkSumFrame_received}|${checkSumFrame_calculated}`);
		// Plausi
		...
		this.log.silly(`[#checkDataFrame] Messagelength: ${msgLen}`);
						...
						for (let j = 0; j < resultFilter.length; j++) {
							...
							switch (result.rules) {
								...
								case 5: {
									...
									for (j = 0; j < 10; j++) {
										serialnr += (modbus_data.readUInt8(j) - 48) * 10 ** (9 - j);
									}
									if (debugging) {
										console.log(
