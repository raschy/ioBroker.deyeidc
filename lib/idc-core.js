...
module.exports = class idcCore {
	...
	checkDataFrame(data) {
		...
		if (logging) this.log.silly(`[#checkDataFrame] Data Buffer ${bufferInHex}`);
		...
		if (logging) this.log.silly(`[#checkDataFrame] Messagelength: ${msgLen}`);
	...
	readCoils(data) {
		...
		if (data.register > 0 && data.register <= Object.keys(this.Registers).length) {
			...
			const expectedLen = (this.Registers[data.register - 1].registerEnd - this.Registers[data.register - 1].registerStart + 2) * 2;
			...
			if (logging) this.log.silly(`[#readCoils] Startcoil: ${startCoil} ExpectedLength: ${expectedLen} ModbusLength: ${modbus_data.length}`);
				...
				for (let i = 0; i < modbus_data.length / 2; i++) {
					...
					if (resultFilter.length > 0) {
						for (let j = 0; j < resultFilter.length; j++) {
							...
							if (logging) this.log.silly(`[#readCoils] Result: ${JSON.stringify(result)} > [${i}]`);
							switch (result.rules) {
								case 0: {	// raw_signed
									const rawDataLow = modbus_data.readUInt8(i * 2);
									const rawDataHgh = modbus_data.readUInt8(i * 2 + 1);
									const rawData = i + ':' + rawDataLow + ':' + rawDataHgh;
									const bitValue = modbus_data.readInt16BE(i * 2).toString(10);
									if (debugging) console.log(`Register ${startCoil + i} [0] [posBuffer:${i * 2}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${bitValue} ${result.unit} # ${result.factor}`);
								...
								case 1: {	// for 16-bit-unsigned Values
									...
									if (debugging) console.log(`Register ${startCoil + i} [1] [posBuffer:${i * 2}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
								...
								case 2: {	// for 16-bit-signed Values
									...
									if (debugging) console.log(`Register ${startCoil + i} [2] [posBuffer:${i * 2}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 3: {	// for 32-bit-unsigned Values
									const rawDataLow = modbus_data.readUInt8(i * 2);
									const rawDataHgh = modbus_data.readUInt8(i * 2 + 1);
									const rawDataLow1 = modbus_data.readUInt8(i * 2 + 2);
									const rawDataHgh1 = modbus_data.readUInt8(i * 2 + 3);
									const rawData = i + ':' + rawDataLow + ':' + rawDataHgh + ':' + rawDataLow1 + ':' + rawDataHgh1;
									...
									if (debugging) console.log(`Register ${startCoil + i} [3] [posBuffer:${i * 2}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 4: {	// for 32-bit-signed Values
									...
									const value = ((low_word + high_word * 65536) * 10 ** -result.factor).toFixed(result.factor);
									if (debugging) console.log(`Register ${startCoil + i} [4] [posBuffer:${i * 2}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 5: { 	// Serialnumber
									...
									if (debugging) console.log(`Register ${startCoil + i} [5] [posBuffer:${i * 2}] SerialNumber: ${serialnr}`);
								...
								case 6: {	// Temperature
									...
									if (debugging) console.log(`Register ${startCoil + i} [6] [posBuffer:${i * 2}] ${result.name}:> ${result.key} Rohdaten ${rawValue} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 7: {	// Versionsnumber
									...
									if (debugging) console.log(`Register ${startCoil + i} [7] [posBuffer:${i * 2}] ${result.name}:> ${result.key} Rohdaten ${rawByte} => ${versionString} ${result.unit} # ${result.factor}`);
								...
								case 8: {	// SingleBytes (MSB)
									...
									if (debugging) console.log(`Register ${startCoil + i} [8] [posBuffer:${i * 2}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${bitValue} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: bitValue, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 9: {	// SingleBytes (LSB)
									...
									if (debugging) console.log(`Register ${startCoil + i} [9] [posBuffer:${i * 2 + 1}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${bitValue} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: bitValue, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
								case 13: {	// for 32-bit-unsigned Values (inverted bytes)
									...
									const rawData = i + ':' + rawDataLow + ':' + rawDataHgh + ':' + rawDataLow1 + ':' + rawDataHgh1;
									const low_word = modbus_data.readUInt16BE(i * 2);
									const high_word = modbus_data.readUInt16BE(i * 2 + 2);
									console.log(`[${i}] high_word ${high_word} <> low_word ${low_word} `);
									const value = ((low_word + high_word * 65536) * 10 ** -result.factor).toFixed(result.factor);
									if (debugging) console.log(`Register ${startCoil + i} [13] [posBuffer:${i * 2}] Rawdata: ${rawData}; ${result.name}:> ${result.key} => ${value} ${result.unit} # ${result.factor}`);
									const jsonObj = { key: result.key, value: value, unit: result.unit, name: result.name };
									jsonResult.push(jsonObj);
									break;
								}
