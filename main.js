...
class Deyeidc extends utils.Adapter {
  ...
  connectToServer() {
    return new Promise((resolve, reject) => {
      ...
      client.connect(
        { host: this.config.ipaddress, port: this.config.port },
        () => {
      ...
      client.on("error", (error) => {
        ...
        if (
          error.message.indexOf("EHOSTUNREACH") > 1 ||
          error.message.indexOf("ECONNRESET") > 1
        ) {
  ...
  async onData(data) {
    try {
      ...
      if (mb) {
        if (mb.register == 0) {
          ...
          if (this.req <= this.numberRegisterSets) {
            ...
            this.log.debug(
              `Data reception for ${this.req - 1} registersets completed`,
            );
      ...
      if (err.status == "ECNTRLCODE") {
        this.log.silly(
          `${err.message}: Data may be corrupt, therefore discarded`,
        );
  ...
  async requestData(req) {
    if (await this.checkConnected()) {
      try {
        ...
        this.log.silly(
          `Request to register set ${req} > ${this.idc.toHexString(request)}`,
        ); // human readable
  ...
  async computeData() {
    ...
    for (const obj of this.config.computes) {
      ...
      if (response) {
        const key1Index = this.memoryValues.findIndex(
          (element) => element.key == response.key1,
        );
        if (key1Index == -1) {
        ...
        //
        const key2Index = this.memoryValues.findIndex(
          (element) => element.key == response.key2,
        );
        if (key2Index == -1) {
  ...
  async setOfflineDate() {
    if (await this.checkConnected()) {
      ...
      const d = new Date();
      data[0] = parseInt(
        decimalToHex(parseInt(d.getFullYear().toString().substring(2))) +
          decimalToHex(d.getMonth() + 1),
        16,
      );
      data[1] = parseInt(
        decimalToHex(d.getDate()) + decimalToHex(d.getHours()),
        16,
      );
      data[2] = parseInt(
        decimalToHex(d.getMinutes()) + decimalToHex(d.getSeconds()),
        16,
      );
      const request = this.idc.requestFrame(
        req,
        this.idc.modbusWriteFrame(dateControlRegister, data),
      );
      this.log.debug(
        `[setOfflineDate] write: ${req} > ${this.idc.toHexString(request)}`,
      ); // human readable
      this.client.write(request);
    }
  ...
  async checkOnlineDate() {
    if (await this.checkConnected()) {
      ...
      const request = this.idc.requestFrame(
        0,
        this.idc.modbusReadFrame(dateControlRegister),
      );
      this.log.silly(
        `Request to date  ( ddhh ) > ${this.idc.toHexString(request)}`,
      ); // human readable
      this.client.write(request);
    }
  }
  ...
  async setPower(id, state) {
    if (await this.checkConnected()) {
      ...
      const request = this.idc.requestFrame(
        req,
        this.idc.modbusWriteFrame(powerControlRegister, data),
      );
      this.client.write(request);
  ...
  async updateData(data) {
    if (data) {
      for (const obj of data) {
        if (obj.value != "none") {
          const elementIndex = this.memoryValues.findIndex(
  ...
  async checkUserData() {
    ...
    if (validateIP(this.config.ipaddress)) {
      this.log.debug(
        `IP address [${this.config.ipaddress}] seems to be valid.`,
      );
    } else {
    ...
    this.log.info(
      `Retrieving data from the inverter will be done every ${this.executionInterval} seconds`,
    );
    //
